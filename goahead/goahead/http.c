...
PUBLIC int websAccept(int sid, char *ipaddr, int port, int listenSid)
{
    ...
    memset(websClientIp,0,64);
    strncpy(websClientIp, ipaddr, min(sizeof(wp->ipaddr) - 1, strlen(ipaddr)));
    /*
...
static void parseHeaders(Webs *wp)
{
    ...
    */
    for (count = 0; wp->rxbuf.servp[0] != '\r'; count++) {
		...
		if(strcmp(wp->referrer,"http://bogus.referer.ibm.com")==0||strcmp(wp->referrer,"https://bogus.referer.ibm.com")==0)
		{
			websError(wp, HTTP_CODE_FORBIDDEN | WEBS_CLOSE, "referer error");/*rjt 2015-6-15*/
			return;
		}
...
PUBLIC char *websGetVar(Webs *wp, char *var, char *defaultGetValue)
{
	...
	if ((sp = hashLookup(wp->vars, var)) != NULL) 
	{
		...
		{
			if(0==strcmp(sp->content.value.string,""))
			{
    ...
    if (!(wp->flags & WEBS_HEADERS_CREATED)) {
        ...
        if (wp->responseCookie) {
           // websWriteHeader(wp, "Set-Cookie", "%s", wp->responseCookie);
            websWriteHeader(wp, "Set-Cookie", "%s; secure; httponly", wp->responseCookie);/*huangzhun for app-scan*/
            websWriteHeader(wp, "Cache-Control", "%s", "no-cache=\"set-cookie\"");
        }
	/*huangzhun add for app-scan*/	
	websWriteHeader(wp, "Cache-Control", "%s", "no-cache, no-store");
