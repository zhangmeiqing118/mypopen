...
static void thread_list_debug(struct thread_list *list)
{
...
thread_list_add_before(struct thread_list *list,
		       struct thread *point, struct thread *thread)
{
	thread->next = point;
...
/* Return remain time. */
char *thread_timer_remain_second(struct thread *thread)
{
    ...
    buf = XMALLOC(MTYPE_THREAD, len + 1);
    if (NULL == buf) {
        return NULL;
    }
...
#ifdef TIMER_NO_SORT
struct timeval *thread_timer_wait(struct thread_master *m,
...
static unsigned long thread_consumed_time(RUSAGE_T * now, RUSAGE_T * start)
{
...
void thread_call(struct thread *thread)
{
	...
	if (thread_time > 200000L) {
		...
		zlog_err("CPU HOG task %lx ran for %ldms",
			 ...
			 (unsigned long)thread->func, thread_time / 1000L);
	}
#endif /* THREAD_CONSUMED_TIME_CHECK */
}

/* Execute thread */
struct thread *thread_execute(__attribute__((unused))struct thread_master *m,
