...
static void vty_dont_lflow_ahead(struct vty *vty)
{
...
static void vty_write(struct vty *vty, char *buf, size_t nbytes)
{
...
static int vty_telnet_option(struct vty *vty, unsigned char *buf, __attribute__((unused))int nbytes)
{
	...
	case SE:
		{
			...
			case TELOPT_NAWS:
				...
				vty->width = buffer[2];
				vty->height =
				    vty->lines >= 0 ? vty->lines : buffer[4];
				break;
			}
			vty->iac_sb_in_progress = 0;
			return 0;
...
static int vty_flush(struct thread *thread)
{
	...
	if (vty->status == VTY_START || vty->status == VTY_CONTINUE) {
		...
		if (vty->lines == 0)
			buffer_flush_window(vty->obuf, vty->fd, vty->width, 25,
					    0, 1);
		else if (vty->status == VTY_MORELINE)
			buffer_flush_window(vty->obuf, vty->fd, vty->width, 1,
					    erase, 0);
		else
			buffer_flush_window(vty->obuf, vty->fd, vty->width,
					    vty->lines >=
					    0 ? vty->lines : vty->height, erase,
					    0);
...
static int vty_accept(struct thread *thread)
{
	...
	if (p->family == AF_INET6 && vty_ipv6_accesslist_name) {
		if ((acl =
		     access_list_lookup(AFI_IP6, vty_ipv6_accesslist_name))
		    && (access_list_apply(acl, p) == FILTER_DENY)) {
			char *buf;
			zlog(NULL, LOG_INFO, "Vty connection refused from %s",
			     (buf = sockunion_su2str(&su)));
...
void vty_serv_sock_addrinfo(const char *hostname, unsigned short port)
{
	int ret;
	struct addrinfo req;
...
void vty_serv_sock_family(unsigned short port, int family)
{
	int ret;
	union sockunion su;
	int accept_sock;
...
void vty_serv_sock(__attribute__((unused))const char *hostname, unsigned short port, __attribute__((unused))char *path)
{
...
FILE *vty_use_backup_config(char *fullpath)
{
...
/* Read up configuration file from file_name. */
void
vty_read_config(char *config_file, char *config_current_dir, char *config_default_dir)
{
	char *cwd;
	FILE *confp = NULL;
	char *fullpath;
	...
	if (config_file != NULL) {
		if (!IS_DIRECTORY_SEP(config_file[0])) {
			...
			fullpath = XMALLOC(MTYPE_TMP, strlen(cwd) + strlen(config_file) + 2);
		...
		if (confp == NULL) {
			...
			if (confp) {
				...
				fprintf(stderr,	"%s[%d]:can't open configuration file [%s]\n",__func__, __LINE__, config_file);
				exit(1);
			}
		}
	} else {
		...
		if (confp == NULL) {
			...
			if (confp == NULL) {
				...
				if (confp) {
					...
					fprintf(stderr,	"%s[%d]:can't open configuration file [%s]\n", __func__, __LINE__, config_default_dir);
					exit(1);
				}
			} else {
				fullpath = config_default_dir;
			...
			cwd = getcwd(NULL, MAXPATHLEN);
			fullpath = XMALLOC(MTYPE_TMP, strlen(cwd) + strlen(config_current_dir) + 2);
...
extern struct thread_master *g_thread_master;
...
static void vty_event(enum event event, int sock, struct vty *vty)
{
	...
	case VTY_SERV:
		vty_serv_thread =
		    thread_add_read(g_thread_master, vty_accept, vty, sock);
		vector_set_index(Vvty_serv_thread, sock, vty_serv_thread);
		break;
#ifdef VTYSH
	case VTYSH_SERV:
		thread_add_read(g_thread_master, vtysh_accept, vty, sock);
		break;
	case VTYSH_READ:
		thread_add_read(g_thread_master, vtysh_read, vty, sock);
		break;
#endif /* VTYSH */
	case VTY_READ:
		vty->t_read = thread_add_read(g_thread_master, vty_read, vty, sock);
		...
		if (vty->v_timeout) {
			if (vty->t_timeout)
				thread_cancel(vty->t_timeout);
			vty->t_timeout =
			    thread_add_timer(g_thread_master, vty_timeout, vty,
					     vty->v_timeout);
		}
		break;
	case VTY_WRITE:
		if (!vty->t_write)
			vty->t_write =
			    thread_add_write(g_thread_master, vty_flush, vty, sock);
		break;
...
/* Set time out value. */
int exec_timeout(struct vty *vty, char *min_str, char *sec_str)
{
...
DEFUN(no_exec_timeout,
      no_exec_timeout_cmd, "no exec-timeout", NO_STR "Set the EXEC timeout\n")
{
...
{
	if (!vty_accesslist_name
	    || (argc && strcmp(vty_accesslist_name, argv[0]))) {
