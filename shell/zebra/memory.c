...
{
	zlog_info("%s: %s %p %s %d", func, lookup(mstr, type), memory, file,
		  line);
}
...
void show_memory_vty(struct vty *vty, struct memory_list *list)
{
...
ALIAS(show_memory_all,
      show_memory_cmd,
      "show memory", "Show running system information\n" "Memory statistics\n");
