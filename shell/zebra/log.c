...
static void time_print(FILE * fp)
{
	int ret;
	char buf[TIME_BUF];
	time_t clock;
...
void vzlog(struct zlog *zl, int priority, const char *format, va_list * args)
{
	...
	if (zl->flags & ZLOG_SYSLOG)
		vsyslog(priority | zlog_default->facility, format,
			args[ZLOG_SYSLOG_INDEX]);
...
int zlog_set_file(struct zlog *zl, __attribute__((unused))int flags, char *filename)
{
	FILE *fp;
