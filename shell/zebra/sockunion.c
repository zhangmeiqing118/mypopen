...
int sockunion_socket(union sockunion *su)
{
	...
	if (sock < 0) {
		zlog(NULL, LOG_WARNING, "Can't make socket : %s",
		     strerror(errno));
		return -1;
	}
...
sockunion_connect(int fd, union sockunion *peersu, unsigned short port,
		  __attribute__((unused))unsigned int ifindex)
{
	int ret;
	int val;
	union sockunion su;
	       ...
	       union sockunion *su_addr)
{
	...
	if (ret < 0)
		zlog(NULL, LOG_WARNING, "can't bind socket : %s",
		     strerror(errno));
...
#else
int sockopt_reuseport(__attribute__((unused))int sock)
{
	return 0;
}
#endif /* 0 */
...
void sockunion_print(union sockunion *su)
{
	if (su == NULL)
		return;
...
int sockunion_cmp(union sockunion *su1, union sockunion *su2)
{
	...
	if (su1->sa.sa_family == AF_INET) {
		if (ntohl(su1->sin.sin_addr.s_addr) ==
		    ntohl(su2->sin.sin_addr.s_addr))
			return 0;
		if (ntohl(su1->sin.sin_addr.s_addr) >
		    ntohl(su2->sin.sin_addr.s_addr))
			return 1;
