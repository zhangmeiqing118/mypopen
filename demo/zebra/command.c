...
void install_node(struct cmd_node *node, int (*func) (struct vty *))
{
	...
	recode = vector_set_index(g_cmd_vec, node->node, node);
    if (recode) {
        exit(-1);
    }
	node->func = func;
	node->cmd_vector = vector_init(VECTOR_MIN_SIZE);
    if (NULL == node->cmd_vector) {
        exit(-1);
    }
...
void sort_node(vector v)
{
	unsigned int i, j;
	struct cmd_node *cnode;
	vector descvec, cmd_vector;
	...
	for (i = 0; i < v->max; i++) {
		if ((cnode = vector_slot(cmdvec, i)) != NULL) {
			...
			for (j = 0; j < cmd_vector->max; j++) {
				if ((cmd_element = vector_slot(cmd_vector, j)) != NULL) {
					descvec = vector_slot(cmd_element->strvec, vector_max(cmd_element->strvec) - 1);
...
vector cmd_make_strvec(char *string)
{
	...
	while (cp && (*cp != '\0')) {
		start = cp;
		while (cp && !(isspace((int)*cp)) && (*cp != '\r') &&  (*cp != '\n') && (*cp != '\0')) {
			cp++;
        }
		strlen = cp - start;
		...
		*(token + strlen) = '\0';
		recode = vector_set(strvec, token);
        if (recode) {
            vector_free(strvec);
            return NULL;
        }
...
/* New string vector. */
vector cmd_make_descvec(char *string, char *descstr)
{
	int multiple = 0;
	char *sp;
	char *token;
	...
	while (cp && (*cp != '\0')) {
		...
		*(token + len) = '\0';
		...
		desc = malloc(sizeof(struct desc));
		desc->cmd = token;
...
char *cmd_prompt(enum node_type node)
{
    ...
    cnode = vector_slot(g_cmd_vec, node);
    if (NULL == cnode) {
        return NULL;
    }
...
void install_element(enum node_type ntype, struct cmd_element *cmd)
{
	...
	cnode = vector_slot(g_cmd_vec, ntype);
	if (NULL == cnode) {
		printf("Command node %d doesn't exist, please check it\n",	ntype);
		exit(-1);
	}

	recode = vector_set(cnode->cmd_vector, cmd);
    if (recode) {
		printf("Command node %d set failed\n",	ntype);
        exit(-1);
    }
	...
	if (host.log_syslog) {
		...
		if (zlog_default->facility != LOG_DAEMON)
			vty_out(vty, " facility %s",
				syslog_facility_print(zlog_default->facility));
		vty_out(vty, "%s", VTY_NEWLINE);
	}
	if (zlog_default->maskpri != LOG_DEBUG)
		vty_out(vty, "log trap %s%s",
			zlog_priority[zlog_default->maskpri], VTY_NEWLINE);
...
enum match_type cmd_filter_by_completion(char *command, vector v, int index)
{
    int matched;
	unsigned int i, j;
	char *str;
	...
	for (i = 0; i < vector_max(v); i++)
		if ((cmd_element = vector_slot(v, i)) != NULL) {
			if (index >= (int)vector_max(cmd_element->strvec))
...
enum match_type cmd_filter_by_string(char *command, vector v, int index)
{
    int matched;
	unsigned int i, j;
	char *str;
...
int is_cmd_ambiguous(char *command, vector v, int index, enum match_type type)
{
	unsigned int i, j;
	char *str = NULL;
	struct cmd_element *cmd_element;
...
/* Check same string element existence.  If it isn't there return
    1. */
int cmd_unique_string(vector v, char *str)
{
	unsigned int i;
	char *match;
...
int desc_unique_string(vector v, char *str)
{
	unsigned int i;
	struct desc *desc;
...
#define INIT_MATCHVEC_SIZE 10
	...
	for (i = 0; i < (unsigned int )index; i++) {
				...
				if ((cmd_element =
					...
					descvec =
					    vector_slot(cmd_element->strvec,
							...
							(cmd_element->strvec) -
							1);
					for (k = 0; k < vector_max(descvec);
					     k++) {
						struct desc *desc =
						    vector_slot(descvec, k);
		...
		if ((ret =
		     is_cmd_ambiguous(command, cmd_vector, i, match)) == 1) {
			vector_free(cmd_vector);
		...
		if ((cmd_element = vector_slot(cmd_vector, i)) != NULL) {
			...
			if (index >= vector_max(strvec))
				...
				for (j = 0; j < vector_max(descvec); j++) {
					...
					if ((string =
					     cmd_entry_function(vector_slot
								(vline, index),
								desc->cmd)))
...
int cmd_execute_command(vector vline, struct vty *vty, struct cmd_element **cmd)
{
	...
	for (index = 0; index < (int)vector_max(vline); index++) {
		int ret;
			   ...
			   struct cmd_element **cmd)
{
	...
	for (i = 0; i < vector_max(cmd_vector); i++)
		if (vector_slot(cmd_vector, i) != NULL) {
			...
			if (match == vararg_match
			    || index >= (unsigned int )(cmd_element->cmdsize)) {
				matched_element = cmd_element;
				matched_count++;
			} else
				incomplete_count++;
	...
	while (fgets(vty->buf, VTY_BUFSIZ, fp)) {
		...
		if (ret != CMD_SUCCESS && ret != CMD_WARNING) {
			if (vty->node == KEYCHAIN_KEY_NODE) {
				...
				ret =
				    cmd_execute_command_strict(vline, vty,
							       NULL);
...
{
	vty_out(vty, "Zebra %s (%s).%s", ZEBRA_VERSION, host_name, VTY_NEWLINE);
	vty_out(vty, "Copyright 1996-2004.%s", VTY_NEWLINE);
...
DEFUN(no_config_log_stdout,
      ...
      "no log stdout", NO_STR "Logging control\n" "Cancel logging to stdout\n")
{
...
DEFUN(no_config_log_syslog,
      ...
      "no log syslog", NO_STR "Logging control\n" "Cancel logging to syslog\n")
{
...
void cmd_init(int terminal)
{
	...
	g_cmd_vec = vector_init(VECTOR_MIN_SIZE);

	/* Default host value settings. */
	g_host_info.name = NULL;
	g_host_info.password = NULL;
	g_host_info.enable = NULL;
	g_host_info.logfile = NULL;
	g_host_info.config = NULL;
	g_host_info.lines = -1;
	g_host_info.motd = DEFAULT_MOTD;
	/* Install top nodes. */
