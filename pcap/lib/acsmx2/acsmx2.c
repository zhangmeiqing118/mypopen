...
static char *sf[] = {"Full", "Sparse", "Banded", "Sparse-Bands", "Full-Q"};
static char *fsa[] = {"TRIE", "NFA", "DFA"};
...
void acsm_init_summary(void)
{
    ...
    memset(&summary.acsm, 0x00, sizeof(ACSM_STRUCT2));
...
static void *AC_MALLOC(int n, Acsm2MemoryType type)
{
    ...
    if (p) {
        ...
        printf("AC MALLOC error\n");
    }

    return p;
}

static void *AC_MALLOC_DFA(int n, int size)
{
    void *p;
...
static void AC_FREE_DFA(void *p, int n, int size)
{
    if (p) {
        switch (size) {
            case 1:
...
static int List_PutNextState(ACSM_STRUCT2 *acsm, int state, int input, int next_state)
{
    ...
    if (NULL == tnew) {
        MEMASSERT (tnew, "List PutNextState");
        return -1;
    }
 ...
 */
static void List_FreeTransTable(ACSM_STRUCT2 *acsm)
{
    unsigned int i;
    trans_node_t *t, *p;

    if (NULL == acsm->acsmTransTable) {
        return;
    }
    ...
    AC_FREE(acsm->acsmTransTable, sizeof(void*) * acsm->acsmMaxStates, ACSM2_MEMORY_TYPE__TRANSTABLE);
...
static int List_ConvToFull(ACSM_STRUCT2 *acsm, acstate_t state, acstate_t *full)
{
    ...
    tcnt = 0;
    memset(full, 0, acsm->stat_size * acsm->acsmAlphabetSize);
    t = acsm->acsmTransTable[state];
    while (t) {
        switch (acsm->stat_size) {
            case 1:
 ...
 */
static int AddMatchListEntry(ACSM_STRUCT2 *acsm, int state, ACSM_PATTERN2 *px)
{
    ACSM_PATTERN2 * p;
...
static int AddPatternStates(ACSM_STRUCT2 *acsm, ACSM_PATTERN2 *p)
{
    unsigned int state, next, n;
     ...
     */
    for (n = p->n; n > 0; pattern++, n--) {
        if (s_verbose) {
            printf(" find char='%c'\n", *pattern);
        }

        next = List_GetNextState(acsm, state, *pattern);
        if ((next == ACSM_FAIL_STATE2) || (!next)) {
            break;
        }
        state = next;
    }
    ...
    for (; n > 0; pattern++, n--) {
        ...
        acsm->acsmNumStates++;
        if (List_PutNextState(acsm, state, *pattern, acsm->acsmNumStates)) {
            return -1;
        }
        state = acsm->acsmNumStates;
    }
    if (AddMatchListEntry(acsm, state, p)) {
        return -1;
    }
    if (s_verbose) {
 ...
 */
static int Build_NFA(ACSM_STRUCT2 *acsm)
{
    unsigned int r, s, i;
    int fs, next;
    QUEUE q, *queue;
    acstate_t     *FailState;
    ACSM_PATTERN2 **MatchList;
    ...
    FailState = acsm->acsmFailState;
    MatchList = acsm->acsmMatchList;
 ...
 */
static int Convert_NFA_To_DFA(ACSM_STRUCT2 *acsm)
{
    unsigned int i, r, s, cFailState;
    QUEUE  q, *queue;
    acstate_t *FailState;
    ...
    while (queue_count(queue)) {
        ...
        for (i = 0; i < acsm->acsmAlphabetSize; i++) {
            s = List_GetNextState(acsm, r, i);
            if (s && ((acstate_t)s != ACSM_FAIL_STATE2)) {
                if (queue_add(queue, s)) {
                    return -1;
                }
            } else {
                cFailState = List_GetNextState(acsm, FailState[r], i);
                if (cFailState && ((acstate_t)cFailState != ACSM_FAIL_STATE2)) {
...
static int Conv_List_To_Full(ACSM_STRUCT2 *acsm)
{
    ...
    NextState = acsm->acsmNextState;
    for (k = 0; k < (acstate_t)acsm->acsmNumStates; k++) {
        p = AC_MALLOC_DFA(acsm->stat_size * (acsm->acsmAlphabetSize + 2), acsm->stat_size);
        if (p == NULL) {
            return -1;
        }
...
{
    unsigned int cnt, m, k, i;
    acstate_t  *p, state, maxstates;
    acstate_t **NextState;
    ...
    for (k = 0; k < acsm->acsmNumStates; k++) {
        ...
        for (i = 0; i < acsm->acsmAlphabetSize; i++) {
            state = full[i];
            if (state && (state != ACSM_FAIL_STATE2)) {
                cnt++;
            }
        }
        ...
        if ((k == 0) || (cnt > acsm->acsmSparseMaxRowNodes)) {
            p = AC_MALLOC_DFA(sizeof(acstate_t) * (acsm->acsmAlphabetSize + 2), sizeof(acstate_t));
            if (NULL == p) {
                return -1;
            }
            ...
            p = AC_MALLOC_DFA(sizeof(acstate_t) * (3 + 2 * cnt), sizeof(acstate_t));
            if (NULL == p) {
                return -1;
            }
...
static int Conv_Full_DFA_To_Banded(ACSM_STRUCT2 *acsm)
{
    ...
    for (k = 0; k < acsm->acsmNumStates; k++) {
        ...
        for (i = (unsigned)first; i <= (unsigned)last; i++) {
            p[m++] = full[i];
        }
 ...
 */
static int calcSparseBands(acstate_t *next, unsigned int *begin, unsigned int *end, unsigned int asize, unsigned int zmax)
{
    unsigned int i, nbands, zcnt, last;
    acstate_t state;
...
static int Conv_Full_DFA_To_SparseBands(ACSM_STRUCT2 *acsm)
{
    ...
    unsigned int cnt, zcnt, nbands;
    unsigned int band_begin[MAX_ALPHABET_SIZE];
    unsigned int band_end[MAX_ALPHABET_SIZE];
    acstate_t *p;
    acstate_t **NextState;
    acstate_t full[MAX_ALPHABET_SIZE];

    zcnt = acsm->acsmSparseMaxZcnt;
    NextState = acsm->acsmNextState;
    for (k = 0; k < acsm->acsmNumStates; k++) {
        cnt = 0;
        List_ConvToFull(acsm, (acstate_t)k, full);
...
static void Print_DFA_MatchList(ACSM_STRUCT2 *acsm, int state)
{
    ...
    for (mlist = acsm->acsmMatchList[state]; mlist; mlist = mlist->next) {
        if (mlist->nocase) {
            ...
            printf("%.*s ", mlist->n, mlist->casepatrn);
        }
    }
...
{
    unsigned int  k, i;
    acstate_t state, n, fmt, index, nb;
    acstate_t **NextState, *p;
...
{
    unsigned int  k, i;
    acstate_t n, fmt, index, nb, bmatch;
    acstate_t **NextState, *p;
    FILE *fp;
 ...
 */
ACSM_STRUCT2 *acsmNew2(user_free_t user_free, option_free_t option_free, neglist_free_t neglist_free)
{
    ACSM_STRUCT2 * p;
    ...
    p->user_free              = user_free;
    p->option_free        = option_free;
    p->neglist_free       = neglist_free;
...
int acsmAddPattern2(ACSM_STRUCT2 *p, unsigned char *pat, int n, int nocase, int offset, int depth, int negative, void *id, int iid)
{
    ...
    memset(plist, 0x00, sizeof(ACSM_PATTERN2));

    plist->patrn = (unsigned char *)AC_MALLOC(n + 1, ACSM2_MEMORY_TYPE__PATTERN);
    if (NULL == plist->patrn) {
        MEMASSERT(plist->patrn, "acsmAddPattern");
        AC_FREE(plist, sizeof(ACSM_PATTERN2), ACSM2_MEMORY_TYPE__PATTERN);
        return -1;;
    }
    ConvertCaseEx(plist->patrn, pat, n + 1);

    plist->casepatrn = (unsigned char *)AC_MALLOC(n + 1, ACSM2_MEMORY_TYPE__PATTERN);
    if (NULL == plist->casepatrn) {
        MEMASSERT (plist->casepatrn, "acsmAddPattern");
        AC_FREE(plist->patrn, n, ACSM2_MEMORY_TYPE__PATTERN);
        AC_FREE(plist, sizeof(ACSM_PATTERN2), ACSM2_MEMORY_TYPE__PATTERN);
        return -1;
    }
    memcpy(plist->casepatrn, pat, n + 1);
    ...
    p->numPatterns++;
    ///printf("patter:%s, casepatrn\n", plist->patrn, plist->casepatrn);
...
int acsmAddKey2(ACSM_STRUCT2 *p, unsigned char *key, int klen, int nocase, void *data)
{
    ...
    if (NULL == plist->patrn) {
        MEMASSERT (plist->patrn, "acsmAddKey2");
        AC_FREE(plist, sizeof(ACSM_PATTERN2), ACSM2_MEMORY_TYPE__PATTERN);
        return -1;
    }
    memcpy (plist->patrn, key, klen);

    plist->casepatrn = (unsigned char *)AC_MALLOC(klen, ACSM2_MEMORY_TYPE__PATTERN);
    if (NULL == plist->casepatrn) {
        MEMASSERT (plist->casepatrn, "acsmAddPattern");
        AC_FREE(plist->patrn, klen, ACSM2_MEMORY_TYPE__PATTERN);
        AC_FREE(plist, sizeof(ACSM_PATTERN2), ACSM2_MEMORY_TYPE__PATTERN);
        return -1;
    }
...
static int acsmBuildMatchStateTrees2(ACSM_STRUCT2 * acsm, build_tree_t build_tree, neglist_func_t neglist_func)
{
    unsigned int i, cnt;
    ACSM_PATTERN2  **MatchList;
    ACSM_PATTERN2 *mlist;
    ...
    for (i = 0; i < acsm->acsmNumStates; i++) {
        for (mlist = MatchList[i]; mlist; mlist = mlist->next) {
            if (mlist->udata) {
                if (mlist->negative) {
                    neglist_func(mlist->udata, &(MatchList[i]->neg_list));
                } else {
...
static inline int _acsmCompile2(ACSM_STRUCT2 *acsm)
{
    ...
    memset(acsm->acsmTransTable, 0x00, sizeof(trans_node_t *) * acsm->acsmMaxStates);

    if (s_verbose) {
        printf("ACSMX-Max Memory-TransTable Setup: %d bytes, %d states, %d active states\n", acsm2_total_memory, acsm->acsmMaxStates, acsm->acsmNumStates);
    }
    ...
    if (NULL == acsm->acsmMatchList) {
        MEMASSERT(acsm->acsmMatchList, "acsmCompile");
        AC_FREE(acsm->acsmTransTable, sizeof(trans_node_t *) * acsm->acsmMaxStates, ACSM2_MEMORY_TYPE__TRANSTABLE);
        return -1;
    }
    memset(acsm->acsmMatchList, 0x00, sizeof(ACSM_PATTERN2 *) * acsm->acsmMaxStates);

    if (s_verbose) {
        printf("ACSMX-Max Memory- MatchList Table Setup: %d bytes, %d states, %d active states\n", acsm2_total_memory, acsm->acsmMaxStates, acsm->acsmNumStates);
        printf("ACSMX-Max Memory-Table Setup: %d bytes, %d states, %d active states\n", acsm2_total_memory, acsm->acsmMaxStates, acsm->acsmNumStates);
    }
    ...
    for (plist = acsm->acsmPatterns; plist; plist = plist->next) {
        ...
        if (AddPatternStates(acsm, plist)) {
            List_FreeTransTable(acsm);
            AC_FREE(acsm->acsmMatchList, sizeof(ACSM_PATTERN2 *) * acsm->acsmMaxStates, ACSM2_MEMORY_TYPE__MATCHLIST);
            return -1;
        }
    }
    ...
    acsm->acsmNextState = (acstate_t **)AC_MALLOC_DFA(sizeof(acstate_t *) * acsm->acsmNumStates, acsm->stat_size);
    if (NULL == acsm->acsmNextState) {
    ...
    if (s_verbose) {
        printf("ACSMX-Max Trie List Memory : %d bytes, %d states, %d active states\n", acsm2_total_memory, acsm->acsmMaxStates, acsm->acsmNumStates);
    ...
    if ((acsm->acsmFSA == FSA_DFA) || (acsm->acsmFSA == FSA_NFA)) {
        ...
        if (Build_NFA(acsm)) {
            ...
            AC_FREE(acsm->acsmMatchList, sizeof(ACSM_PATTERN2 *) * acsm->acsmMaxStates, ACSM2_MEMORY_TYPE__MATCHLIST);
            AC_FREE(acsm->acsmFailState, sizeof(acstate_t) * acsm->acsmNumStates, ACSM2_MEMORY_TYPE__FAILSTATE);
            AC_FREE(acsm->acsmNextState, sizeof(acstate_t *) * acsm->acsmNumStates, acsm->stat_size);
        ...
        if (s_verbose) {
            ...
            printf("ACSMX-Max NFA List Memory  : %d bytes, %d states / %d active states\n", acsm2_total_memory, acsm->acsmMaxStates, acsm->acsmNumStates);
            List_PrintTransTable(acsm);
        }
    }
    ...
    if (acsm->acsmFSA == FSA_DFA) {
        ...
        if (s_verbose) {
            ...
            printf("ACSMX-Max NFA-DFA List Memory  : %d bytes, %d states / %d active states\n", acsm2_total_memory, acsm->acsmMaxStates, acsm->acsmNumStates);
            List_PrintTransTable(acsm);
        }
    }
    ...
    if (acsm->acsmFormat == ACF_SPARSE) {
        ...
        if (s_verbose) {
            printf ("ACSMX-Max Memory-Sparse: %d bytes, %d states, %d active states\n", acsm2_total_memory, acsm->acsmMaxStates, acsm->acsmNumStates);
            Print_DFA(acsm);
        }
        ...
        if (s_verbose) {
            printf("ACSMX-Max Memory-banded: %d bytes, %d states, %d active states\n", acsm2_total_memory, acsm->acsmMaxStates, acsm->acsmNumStates);
            Print_DFA(acsm);
        }
        ...
        if (s_verbose) {
            printf("ACSMX-Max Memory-sparse-bands: %d bytes, %d states, %d active states\n", acsm2_total_memory, acsm->acsmMaxStates, acsm->acsmNumStates);
            Print_DFA(acsm);
        }
        ...
        if (s_verbose) {
            printf("ACSMX-Max Memory-Full: %d bytes, %d states, %d active states\n", acsm2_total_memory, acsm->acsmMaxStates, acsm->acsmNumStates);
            Print_DFA(acsm);
        }
    ...
    if (s_verbose) {
        printf("ACSMX-Max Memory-Final: %d bytes, %d states, %d active states\n", acsm2_total_memory, acsm->acsmMaxStates, acsm->acsmNumStates);
    }
...
int acsmCompile2(ACSM_STRUCT2 *acsm, build_tree_t build_tree, neglist_func_t neglist_func)
{
    if (_acsmCompile2(acsm)) {
        return -1;
    }

    if (build_tree && neglist_func) {
        acsmBuildMatchStateTrees2(acsm, build_tree, neglist_func);
    }
 ...
 */
static inline int acsmSearchSparseDFA(ACSM_STRUCT2 *acsm, unsigned char *Tx, int n, match_func_t match, void *data, int *current_state)
{
    int index, nfound;
    acstate_t state;
    ACSM_PATTERN2   *mlist;
...
static inline unsigned _process_queue(PMQ *q, match_func_t match, void *data )
{
    unsigned int i;
    ACSM_PATTERN2 * mlist;
...
static inline int acsmSearchSparseDFA_Full_q(ACSM_STRUCT2 *acsm, unsigned char *T, int n, match_func_t match, void *data, int *current_state)
{
    unsigned char *Tend;
    int sindex;
    acstate_t state;
    ...
    switch (acsm->stat_size) {
        ...
        default:
            {
                acstate_t *ps;
                acstate_t **NextState = (acstate_t **)acsm->acsmNextState;
                AC_SEARCH_Q;
            }
            break;
    }

    if (MatchList[*current_state]) {
        _add_queue(&(acsm->q), MatchList[*current_state]);
    }
...
static inline int acsmSearchSparseDFA_Full_q_all(ACSM_STRUCT2 *acsm, const unsigned char *T, int n, match_func_t match, void *data, int *current_state)
{
...
static inline int acsmSearchSparseDFA_Full(ACSM_STRUCT2 *acsm, unsigned char *Tx, int n, match_func_t match, void *data, int *current_state)
{
    ACSM_PATTERN2 *mlist;
    unsigned char *Tend;
    unsigned char *T;
...
static inline int acsmSearchSparseDFA_Full_All(ACSM_STRUCT2 *acsm, const unsigned char *Tx, int n, match_func_t match, void *data, int *current_state)
{
    ACSM_PATTERN2 *mlist;
    const unsigned char   *Tend;
 ...
 */
static inline int acsmSearchSparseDFA_Banded(ACSM_STRUCT2 *acsm, unsigned char *Tx, int n, match_func_t match, void *data, int *current_state)
{
    acstate_t         state;
    unsigned char   * Tend;
    unsigned char   * T;
    int index, sindex, nfound;
    acstate_t      **NextState;
    ACSM_PATTERN2  **MatchList;
    ACSM_PATTERN2   *mlist;
    acstate_t       *ps;
 ...
 */
static inline int acsmSearchSparseNFA(ACSM_STRUCT2 *acsm, unsigned char *Tx, int n, match_func_t match, void *data, int *current_state)
{
    int index, nfound;
    unsigned char *T, *Tend, Tchar;
    acstate_t state, nstate;
    acstate_t **NextState;
    acstate_t *FailState;
    ACSM_PATTERN2 *mlist;
    ACSM_PATTERN2  **MatchList;
 ...
 *   Search Function
 */
int acsmSearch2(ACSM_STRUCT2 *acsm, unsigned char *Tx, int n, match_func_t match, void *data, int *current_state)
{
    switch (acsm->acsmFSA) {
        case FSA_DFA:
            if (acsm->acsmFormat == ACF_FULL) {
                return acsmSearchSparseDFA_Full(acsm, Tx, n, match, data, current_state);
            } else if (acsm->acsmFormat == ACF_FULLQ) {
                return acsmSearchSparseDFA_Full_q(acsm, Tx, n, match, data, current_state);
            } else if (acsm->acsmFormat == ACF_BANDED) {
                return acsmSearchSparseDFA_Banded(acsm, Tx, n, match, data, current_state);
            } else {
 ...
 *   Search Function
 */
int acsmSearchAll2(ACSM_STRUCT2 *acsm, unsigned char *Tx, int n, match_func_t match, void *data, int *current_state)
{
    switch (acsm->acsmFSA) {
        case FSA_DFA:
            if (acsm->acsmFormat == ACF_FULL) {
                return acsmSearchSparseDFA_Full_All(acsm, Tx, n, match, data, current_state);
            } else if (acsm->acsmFormat == ACF_FULLQ) {
                return acsmSearchSparseDFA_Full_q_all(acsm, Tx, n, match, data, current_state);
            } else if (acsm->acsmFormat == ACF_BANDED) {
                return acsmSearchSparseDFA_Banded(acsm, Tx, n, match, data, current_state);
            } else {
                return acsmSearchSparseDFA(acsm, Tx, n, match, data, current_state);
...
void acsmFree2(ACSM_STRUCT2 *acsm)
{
    ...
    for (i = 0; i < acsm->acsmNumStates; i++) {
        ...
        while (mlist) {
            tmp = mlist;
            mlist = mlist->next;
            if (tmp->rule_option_tree && acsm->option_free) {
                acsm->option_free(&(tmp->rule_option_tree));
            }

            if (tmp->neg_list && acsm->neglist_free) {
                acsm->neglist_free(&(tmp->neg_list));
            }

            AC_FREE(tmp, sizeof(ACSM_PATTERN2), ACSM2_MEMORY_TYPE__MATCHLIST);
        }

        AC_FREE_DFA(acsm->acsmNextState[i], acsm->stat_size * (acsm->acsmAlphabetSize + 2), acsm->stat_size);
    }
    AC_FREE(acsm->acsmMatchList, sizeof(ACSM_PATTERN2 *) * acsm->acsmMaxStates, ACSM2_MEMORY_TYPE__MATCHLIST);
    AC_FREE(acsm->acsmFailState, sizeof(acstate_t) * acsm->acsmNumStates, ACSM2_MEMORY_TYPE__FAILSTATE);
    AC_FREE_DFA(acsm->acsmNextState, sizeof(acstate_t *) * acsm->acsmNumStates, acsm->stat_size);


    for (mlist = acsm->acsmPatterns; mlist;) {
        tmp = mlist;
        mlist = mlist->next;
        if (acsm->user_free && tmp->udata) {
            acsm->user_free(tmp->udata);
        }

        AC_FREE(tmp->patrn, tmp->n + 1, ACSM2_MEMORY_TYPE__PATTERN);
        AC_FREE(tmp->casepatrn, tmp->n + 1, ACSM2_MEMORY_TYPE__PATTERN);
        AC_FREE(tmp, sizeof(ACSM_PATTERN2), ACSM2_MEMORY_TYPE__PATTERN);
    }

    AC_FREE(acsm, sizeof(ACSM_STRUCT2), ACSM2_MEMORY_TYPE__NONE);
}
...
int MatchFound(void* id, void *treee, int index, void *data, void *neg_list)
{
    fprintf (stdout, "id:%s, index:%d, data:%s\n", (char *) id, index, (char *)data);
...
int main (int argc, char **argv)
{
    ...
    acsm = acsmNew2(acsmx2_free, objfree, objfree);
    if (!acsm) {
    ...
    for (i = 2; i < argc; i++) {
        ...
        char *userdata = malloc(strlen(p) + 1);
        if (NULL == userdata) {
            return -1;
        }
        memcpy(userdata, p, strlen(p));
        userdata[strlen(p)]= '\0';

        acsmAddPattern2(acsm, p, strlen(p), nc, 0, 0, 0, userdata, i - 2);
    }

    acsmCompile2(acsm, buildtree, neglist);
    ...
    acsmSearch2(acsm, text, strlen(text), MatchFound, NULL, &current_state);
    ///List_PrintTransTable(acsm);
